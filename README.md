...
or with precomputed hills input:
    ...
    biosaur2 path_to_input.hills.npz
...
-write_hills: Add hills output if added as the parameter. Output format is controlled by `--hills_format`.

--hills_format: Format for hills output generated by `-write_hills`. Supported values: `tsv`, `npz`. Default = `tsv`

--hills_float: Floating-point precision for all float fields in NPZ hills output. Supported values: `float32`, `float64`. Default = `float32`
