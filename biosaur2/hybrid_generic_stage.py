...
def _run_generic_enabled_stage(**kwargs):
    ...
    ingestion = kwargs["ingestion"]
    strict_contexts = kwargs["strict_contexts"]
    args = kwargs["args"]
    audit_by_event = kwargs["audit_by_event"]
    strict_index = kwargs["strict_index"]
    strict_hill_claims = kwargs["strict_hill_claims"]
    residual_ledger = kwargs["residual_ledger"]
    residual_allocation_status_counts = kwargs["residual_allocation_status_counts"]
    strict_ownership = kwargs["strict_ownership"]
    strict_quant_rows = kwargs["strict_quant_rows"]
    recovered = kwargs["recovered"]
    recovered_quant_rows = kwargs["recovered_quant_rows"]
    local_candidate_cache_telemetry = kwargs["local_candidate_cache_telemetry"]
    next_feature_id = kwargs["next_feature_id"]
    final_strict_detector = kwargs["final_strict_detector"]
    generic_recovered_feature_rows = kwargs["generic_recovered_feature_rows"]
    generic_recovered_quant_rows = kwargs["generic_recovered_quant_rows"]
...
def _run_relaxed_generic_recovery(state):
    ...
    local_ppm = state["local_ppm"]
    local_rt_tolerance = state["local_rt_tolerance"]
    width_limit = state["width_limit"]
    local_competitions = state["local_competitions"]
    local_status_counts = state["local_status_counts"]
    q_value_max = state["q_value_max"]
    ...
    if bool(args.get("relaxed_ms2_feature", False)):
        ...
        (
            ...
            relaxed_recovered, relaxed_strict_competition, next_feature_id,
            local_status_counts,
            ...
            target_strict_protection_counts, decoy_strict_protection_counts,
            relaxed_target_local, relaxed_decoy_local, relaxed_recovered,
            relaxed_strict_competition, local_status_counts,
        )
    state["next_feature_id"] = next_feature_id
    state["relaxed_target_local"] = relaxed_target_local
    state["relaxed_decoy_local"] = relaxed_decoy_local
    state["relaxed_competitions"] = relaxed_competitions
    state["relaxed_recovered"] = relaxed_recovered
    state["relaxed_strict_competition"] = relaxed_strict_competition
    state["local_status_counts"] = local_status_counts
    return _finalize_generic_stage(state)
...
def _apply_relaxed_generic_results(
    ...
    decoy_strict_protection_counts, relaxed_target_local, relaxed_decoy_local,
    relaxed_recovered, relaxed_strict_competition, local_status_counts,
):
...
def _finalize_generic_stage(state):
    ...
    generic_recovered = state["generic_recovered"]
    generic_score_weights = state["generic_score_weights"]
    generic_started = state["generic_started"]
    local_events = state["local_events"]
    local_input_status_counts = state["local_input_status_counts"]
    local_q_family_counts = state["local_q_family_counts"]
    target_local = state["target_local"]
    decoy_local = state["decoy_local"]
    local_competitions = state["local_competitions"]
    local_ppm = state["local_ppm"]
    width_limit = state["width_limit"]
    q_value_max = state["q_value_max"]
    local_status_counts = state["local_status_counts"]
    relaxed_target_local = state["relaxed_target_local"]
    relaxed_decoy_local = state["relaxed_decoy_local"]
    relaxed_competitions = state["relaxed_competitions"]
    relaxed_recovered = state["relaxed_recovered"]
    relaxed_strict_competition = state["relaxed_strict_competition"]
...
def run_generic_stage(
    *, run_id, ingestion, strict_contexts, args, audit_by_event, strict_index,
    strict_hill_claims, residual_ledger, residual_allocation_status_counts,
    strict_ownership, strict_quant_rows, recovered, recovered_quant_rows,
    local_candidate_cache_telemetry, next_feature_id, final_strict_detector,
):
    generic_recovered_feature_rows = []
    generic_recovered_quant_rows = []
    generic_recovered = []
    generic_score_weights = dict(GENERIC_ASSOCIATION_SCORE_WEIGHTS)
    if not args.get("generic_ms2_refine", True):
        return {
    ...
    return _run_generic_enabled_stage(
        run_id=run_id, ingestion=ingestion, strict_contexts=strict_contexts,
        args=args, audit_by_event=audit_by_event, strict_index=strict_index,
        strict_hill_claims=strict_hill_claims, residual_ledger=residual_ledger,
        residual_allocation_status_counts=residual_allocation_status_counts,
        strict_ownership=strict_ownership, strict_quant_rows=strict_quant_rows,
        recovered=recovered, recovered_quant_rows=recovered_quant_rows,
        local_candidate_cache_telemetry=local_candidate_cache_telemetry,
        next_feature_id=next_feature_id, final_strict_detector=final_strict_detector,
        generic_recovered_feature_rows=generic_recovered_feature_rows,
        generic_recovered_quant_rows=generic_recovered_quant_rows,
        generic_recovered=generic_recovered, generic_score_weights=generic_score_weights,
    )
