...
peaks, constrained to 1-30 GiB. A new run keeps that estimate reserved until
both its wall time and aggregate owned-process CPU time reach three minutes.
The existing heartbeat aggregates CPU seconds without extra process scans.
Older runs are represented by host available memory and do not reserve possible
future growth. In this mode, `--max-memory` is an integer-GiB host-use ceiling
and the manager retains the larger of 8 GiB, 5% physical RAM, or the difference
between physical RAM and that ceiling. A safety-floor breach terminates newest
work and requeues it; the third preemption is a final memory failure. Use
`--scheduler-resource-mode detailed` to retain complete Project PSS accounting.
Each run's DuckDB output connection is limited to its effective run worker
allocation.
