...
peaks, constrained to 1-30 GiB. A new run keeps that estimate reserved for its
first three minutes, regardless of RSS heartbeat observations. Older runs are
represented by host available memory and do not reserve possible future growth.
In this mode, `--max-memory` is an integer-GiB host-use ceiling and the manager
retains the larger of 8 GiB, 5% physical RAM, or the difference between
physical RAM and that ceiling. A safety-floor breach terminates newest work and
requeues it; the third preemption is a final memory failure. Use
